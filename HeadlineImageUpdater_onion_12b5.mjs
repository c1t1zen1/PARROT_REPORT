...
async function fetchOnionHeadline() {
    try {
        ...
        const headline = $('h2:contains("Onion")').next().find('li a').eq(11).text().trim();

        if (headline) {
            return headline;
        } else {
            console.error("Failed to extract the Onion 12 headline using cheerio.");
            return null;
        }
    } catch (error) {
        console.error("Error fetching the Onion 12 headline:", error);
        return null;
    }
}
...
async function generateImagesFromHeadline() {
    ...
    if (headline) {
      ...
      try {
        ...
        console.log("Onion 12 Headline:", directory);

        const indexContent = fs.readFileSync('./index.html', 'utf-8');
        const imgTag = `<center><img src="${imagePath}" alt="Generated Image" width="98%"></center><br>`; // Added <br> here

        const $ = cheerio.load(indexContent);
        const FarkHeadlineElement = $('h2:contains("Onion")').next().find('li a').eq(11);
        FarkHeadlineElement.after(imgTag);

        fs.writeFileSync('./index.html', $.html());
            console.log("index.html updated with generated image before the Onion 12 headline.");
        } catch (error) {
            console.error("Error generating images:", error);
            
            // Handle the error by generating a new image with the given prompt
            try {
                const fallbackImageFiles = await generateImageFiles("photorealistic Style, Excited Parrot wearing a fedora and dressed like a reporter with a notepad during an investigation");
        ...
        console.log("Failed to fetch the Onion 12 headline.");
    }
}
