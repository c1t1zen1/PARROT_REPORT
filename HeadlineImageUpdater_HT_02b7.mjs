...
async function generateImagesFromHeadline() {
    ...
    if (headline) {
      try {
        ...
        console.log(`Hard Times 2 Headline: ${sanitizedHeadline}\n`);
        ...
        fs.writeFileSync('./index2.html', $.html());
            console.log(`Updated Hard Times 2 images: ${directory}\n`);
        } catch (error) {
          ...
          setTimeout(async () => {
            try {
                ...
                const $ = cheerio.load(indexContent);
                const FarkHeadlineElement = $('h2:contains("Hard Times")').next().find('li a').eq(1);
                const imgTag = `<center><img src="data:image/jpeg;base64,${fallbackImageData}" onerror="this.src='PARROT_UFO.jpg';" alt="Fallback Image" width="98%"></center><br>`;
                FarkHeadlineElement.before(imgTag);
                fs.writeFileSync('./index2.html', $.html());
