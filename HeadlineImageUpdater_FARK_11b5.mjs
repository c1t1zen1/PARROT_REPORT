...
async function fetchFarkHeadline() {
    try {
        ...
        const headline = $('h2:contains("Fark")').next().find('li a').eq(10).text().trim();

        if (headline) {
            return headline;
        } else {
            console.error("Failed to extract the Fark 11 headline using cheerio.");
            return null;
        }
    } catch (error) {
        console.error("Error fetching the Fark 11 headline:", error);
        return null;
    }
}
...
async function generateImagesFromHeadline() {
    ...
    if (headline) {
      ...
      try {
        ...
        console.log("Fark 11 Headline:", directory);

        const indexContent = fs.readFileSync('./index.html', 'utf-8');
        const imgTag = `<center><br><img src="${imagePath}" alt="Generated Image" width="98%"></center>`; // Added <br> here

        const $ = cheerio.load(indexContent);
        const FarkHeadlineElement = $('h2:contains("Fark")').next().find('li a').eq(10);
        FarkHeadlineElement.after(imgTag);

        fs.writeFileSync('./index.html', $.html());
            console.log("index.html updated with generated image after the Fark 10 headline.");
        } catch (error) {
        ...
        console.log("Failed to fetch the Fark 11 headline.");
    }
}
