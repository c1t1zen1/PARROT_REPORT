...
async function fetchFarkHeadline() {
    try {
        ...
        const headline = $('h2:contains("Fark")').next().find('li a').eq(12).text().trim();
        if (headline) {
            return headline;
        } else {
            console.error("Failed to extract the Fark 13 headline using cheerio.");
            return null;
        }
    } catch (error) {
...
async function generateImagesFromHeadline() {
    ...
    if (headline) {
      try {
        ...
        console.log(`Fark 13 Headline: ${sanitizedHeadline}\n`);
        ...
        const FarkHeadlineElement = $('h2:contains("Fark")').next().find('li a').eq(12);
        FarkHeadlineElement.after(imgTag);
        fs.writeFileSync('./index2.html', $.html());
            console.log(`Updated Fark 13 images: ${directory}\n`);
        } catch (error) {
          console.error(`Error generating images: ${error}\n`);         
          // Handle the error by generating a new image with the given prompt after 5 seconds 
          setTimeout(async () => {
            try {
                const fallbackImageFiles = await generateImageFiles("camo Parrot wearing a fedora and dressed like a reporter with a notepad in jungle");
        ...
        console.log(`Failed to fetch the Fark 13 headline.\n`);
    }
}
