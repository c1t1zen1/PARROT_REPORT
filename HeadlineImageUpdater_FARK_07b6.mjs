    ...
    if (headline) {
      try {
        ...
        fs.writeFileSync('./index2.html', $.html());
            console.log(`Updated Fark 7 images: ${directory}\n`);
        } catch (error) {
          console.error(`Error generating images: ${error}\n`);         
          // Handle the error by generating a new image with the given prompt after 5 seconds 
          setTimeout(async () => {
            try {
                const fallbackImageFiles = await generateImageFiles("side eye Parrot wearing a fedora and dressed like a reporter with a notepad in a huge tree");
