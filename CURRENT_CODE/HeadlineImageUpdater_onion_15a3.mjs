...
async function fetchOnionHeadline() {
    try {
        ...
        const headline = $('h2:contains("Onion")').next().find('li a').eq(14).text().trim();
        if (headline) {
            return headline;
        } else {
            console.error("Failed to extract the Onion 15 headline using cheerio.");
            return null;
        }
    } catch (error) {
        console.error("Error fetching the Onion 15 headline:", error);
        return null;
    }
}
...
async function generateImagesFromHeadline() {
    ...
    if (headline) {
      try {
        ...
        console.log(`Onion 15 Headline: ${sanitizedHeadline}\n`);
        ...
        const FarkHeadlineElement = $('h2:contains("Onion")').next().find('li a').eq(14);
        FarkHeadlineElement.before(imgTag);
        fs.writeFileSync('./index2.html', $.html());
            console.log(`Updated Onion 15 images: ${directory}\n`);
        } catch (error) {
        ...
        console.log(`Failed to fetch the Onion 15 headline.\n`);
    }
}
