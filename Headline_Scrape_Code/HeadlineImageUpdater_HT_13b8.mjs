...
async function fetchHardTimesHeadline() {
    try {
        ...
        const headline = $('h2:contains("Hard Times")').next().find('li a').eq(12).text().trim();
        if (headline) {
            return headline;
        } else {
            console.error("Failed to extract the Hard Times 13 headline using cheerio.");
            return null;
        }
    } catch (error) {
...
async function generateImagesFromHeadline() {
    ...
    if (headline) {
      try {
        ...
        console.log(`Hard Times 13 Headline: ${sanitizedHeadline}\n`);
        ...
        const FarkHeadlineElement = $('h2:contains("Hard Times")').next().find('li a').eq(12);
        FarkHeadlineElement.before(imgTag);
        fs.writeFileSync('./index2.html', $.html());
            console.log(`Updated Hard Times 13 images: ${directory}\n`);
        } catch (error) {
        ...
        console.log(`Failed to fetch the Hard Times 13 headline.\n`);
    }
}
