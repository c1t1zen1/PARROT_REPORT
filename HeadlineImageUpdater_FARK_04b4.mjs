...
async function fetchFarkHeadline() {
    try {
        ...
        const headline = $('h2:contains("Fark")').next().find('li a').eq(3).text().trim();

        if (headline) {
            return headline;
        } else {
            console.error("Failed to extract the Fark 4 headline using cheerio.");
            return null;
        }
    } catch (error) {
        console.error("Error fetching the Fark 4 headline:", error);
        return null;
    }
}
...
async function generateImagesFromHeadline() {
    ...
    if (headline) {
      // console.log("Fetched Fark 4 Headline:", headline);
      try {
        ...
        console.log("Fark 4 Headline:", sanitizedHeadline);
        ...
        console.log("Fark 4 Headline:", directory);
        ...
        const FarkHeadlineElement = $('h2:contains("Fark")').next().find('li a').eq(3);
        FarkHeadlineElement.after(imgTag);

        fs.writeFileSync('C:/Users/WindowsX/Documents/Code/Bimg/index.html', $.html());
            console.log("index.html updated with generated image after the Fark 4 headline.");
        } catch (error) {
        ...
        console.log("Failed to fetch the Fark 4 headline.");
    }
}
