...
async function generateImagesFromHeadline() {
    ...
    if (headline) {
      // console.log("Fetched Onion 1 Headline:", headline);
      try {
        ...
        console.log("Onion 1 Headline:", directory);
        ...
        fs.writeFileSync('./index2.html', $.html());
            console.log("index.html updated with generated image before the Onion 1 headline.");
        } catch (error) {
            console.error("Error generating images:", error);
            
            // Handle the error by generating a new image with the given prompt
            try {
                const fallbackImageFiles = await generateImageFiles("Film Noir style, Tabloid News Parrot wearing a fedora and dressed like a reporter with a notepad");
