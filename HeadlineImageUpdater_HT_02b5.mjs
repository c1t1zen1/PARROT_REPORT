...
async function fetchHardTimesHeadline() {
    try {
        ...
        const headline = $('h2:contains("Hard Times")').next().find('li a').eq(1).text().trim();

        if (headline) {
            return headline;
        } else {
            console.error("Failed to extract the Hard Times 2 headline using cheerio.");
            return null;
        }
    } catch (error) {
        console.error("Error fetching the HardTimes 2 headline:", error);
        return null;
    }
}
...
async function generateImagesFromHeadline() {
    ...
    if (headline) {
      ...
      try {
        ...
        console.log("Hard Timnes 2 Headline:", directory);
        ...
        const FarkHeadlineElement = $('h2:contains("Hard Times")').next().find('li a').eq(1);
        FarkHeadlineElement.after(imgTag);

        fs.writeFileSync('./index2.html', $.html());
            console.log("index.html updated with generated image before the Hard Times 2 headline.");
        } catch (error) {
        ...
        console.log("Failed to fetch the Hard Times 2 headline.");
    }
}
