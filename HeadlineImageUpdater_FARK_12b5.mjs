...
async function fetchFarkHeadline() {
    try {
        ...
        const headline = $('h2:contains("Fark")').next().find('li a').eq(11).text().trim();

        if (headline) {
            return headline;
        } else {
            console.error("Failed to extract the Fark 12 headline using cheerio.");
            return null;
        }
    } catch (error) {
        console.error("Error fetching the Fark 12 headline:", error);
        return null;
    }
}
...
async function generateImagesFromHeadline() {
    ...
    if (headline) {
      ...
      try {
        ...
        console.log("Fark 12 Headline:", sanitizedHeadline);
        ...
        console.log("Failed to fetch the Fark 12 headline.");
    }
}
