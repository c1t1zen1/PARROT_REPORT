...
async function fetchOnionHeadline() {
    try {
        ...
        const headline = $('h2:contains("Onion")').next().find('li a').eq(16).text().trim();
        if (headline) {
            return headline;
        } else {
            console.error("Failed to extract the Onion 17 headline using cheerio.");
            return null;
        }
    } catch (error) {
        console.error("Error fetching the Onion 17 headline:", error);
        return null;
    }
}
...
async function generateImagesFromHeadline() {
    ...
    if (headline) {
      try {
        ...
        console.log(`Onion 17 Headline: ${sanitizedHeadline}\n`);
        ...
        const FarkHeadlineElement = $('h2:contains("Onion")').next().find('li a').eq(16);
        FarkHeadlineElement.after(imgTag);
        fs.writeFileSync('./index2.html', $.html());
            console.log(`Updated Onion 17 images: ${directory}\n`);
        } catch (error) {
        ...
        console.log(`Failed to fetch the Onion 17 headline.\n`);
    }
}
