...
async function fetchHardTimesHeadline() {
    try {
        ...
        const headline = $('h2:contains("Hard Times")').next().find('li a').eq(5).text().trim();

        if (headline) {
            return headline;
        } else {
            console.error("Failed to extract the Hard Times 6 headline using cheerio.");
            return null;
        }
    } catch (error) {
        console.error("Error fetching the HardTimes 6 headline:", error);
        return null;
    }
}
...
async function generateImagesFromHeadline() {
    ...
    if (headline) {
      ...
      try {
        ...
        console.log("Hard Timnes 6 Headline:", directory);

        const indexContent = fs.readFileSync('./index.html', 'utf-8');
        const imgTag = `<center><img src="${imagePath}" alt="Generated Image" width="98%"></center><br>`; // Added <br> here

        const $ = cheerio.load(indexContent);
        const FarkHeadlineElement = $('h2:contains("Hard Times")').next().find('li a').eq(5);
        FarkHeadlineElement.after(imgTag);

        fs.writeFileSync('./index.html', $.html());
            console.log("index.html updated with generated image before the Hard Times 6 headline.");
        } catch (error) {
        ...
        console.log("Failed to fetch the Hard Times 6 headline.");
    }
}
