...
async function fetchFarkHeadline() {
    try {
        ...
        const headline = $('h2:contains("Fark")').next().find('li a').eq(6).text().trim();

        if (headline) {
            return headline;
        } else {
            console.error("Failed to extract the Fark 7 headline using cheerio.");
            return null;
        }
    } catch (error) {
        console.error("Error fetching the Fark 7 headline:", error);
        return null;
    }
}
...
async function generateImagesFromHeadline() {
    ...
    if (headline) {
      // console.log("Fetched Fark 7 Headline:", headline);
      try {
        ...
        console.log("Fark 7 Headline:", sanitizedHeadline);
        ...
        console.log("Fark 7 Headline:", directory);
        ...
        const FarkHeadlineElement = $('h2:contains("Fark")').next().find('li a').eq(6);
        FarkHeadlineElement.after(imgTag);

        fs.writeFileSync('C:/Users/WindowsX/Documents/Code/Bimg/index.html', $.html());
            console.log("index.html updated with generated image after the Fark 7 headline.");
        } catch (error) {
        ...
        console.log("Failed to fetch the Fark 7 headline.");
    }
}
