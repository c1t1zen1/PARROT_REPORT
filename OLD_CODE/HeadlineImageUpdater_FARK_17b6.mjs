...
async function fetchFarkHeadline() {
    try {
        ...
        const headline = $('h2:contains("Fark")').next().find('li a').eq(16).text().trim();
        if (headline) {
            return headline;
        } else {
            console.error("Failed to extract the Fark 17 headline using cheerio.");
            return null;
        }
    } catch (error) {
...
async function generateImagesFromHeadline() {
    ...
    if (headline) {
      try {
        ...
        console.log(`Fark 17 Headline: ${sanitizedHeadline}\n`);
        ...
        const FarkHeadlineElement = $('h2:contains("Fark")').next().find('li a').eq(16);
        FarkHeadlineElement.after(imgTag);
        fs.writeFileSync('./index2.html', $.html());
            console.log(`Updated Fark 17 images: ${directory}\n`);
        } catch (error) {
        ...
        console.log(`Failed to fetch the Fark 17 headline.\n`);
    }
}
