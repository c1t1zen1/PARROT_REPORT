...
async function generateImagesFromHeadline() {
    ...
    if (headline) {
      ...
      try {
        ...
        console.log("Onion 3 Headline:", directory);
        ...
        fs.writeFileSync('./index.html', $.html());
            console.log("index.html updated with generated image before the Onion 3 headline.");
        } catch (error) {
            console.error("Error generating images:", error);
            
            // Handle the error by generating a new image with the given prompt
            try {
                const fallbackImageFiles = await generateImageFiles("B&W photo Style, Very fashionable Parrot wearing a fedora and dressed like a reporter with a notepad");
